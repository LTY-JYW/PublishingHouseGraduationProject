...
import { ref } from 'vue'
// 导入API
import { categoryGetListAPI, categoryDeleteAPI, categoryAddAPI,categoryUpdataAPI } from '@/api/category'
// 导入API类型
...
// 要修改分类的id
const UpdataId = ref<number>(0)
// 修改一级分类事件函数
const headEdit = async (data:CategoryUpdataType) => {
  isDrawer.value = true
  isAdd.value = false
  UpdataId.value = data.id
  addCategoryData.value.name = data.name
  addCategoryData.value.profile = data.profile
}
// 删除一级分类事件函数
...
const onAddCategory = () => {
  isDrawer.value = true
  isAdd.value = true
  addCategoryData.value.name = ''
  addCategoryData.value.profile = ''
}
// 确认添加图书事件函数
...
<template>
  <pageComponent title="一级分类管理">
    <!-- 添加资讯插槽 -->
    <template #button>
      <el-button plain type="primary" round @click="onAddCategory">添加资讯</el-button>
    </template>
    <!-- 表格部分 -->
    ...
    <el-drawer v-model="isDrawer" title="更新图书" direction="rtl">
      ...
      <el-form size="large" autocomplete="off" :model="addCategoryData" :rules="addCategoryRules" ref="ruleFormRef">
        <!-- 表单数据区域 -->
        <el-form-item prop="name" label="分类名" label-position="top">
          <el-input placeholder="请输入分类名" v-model="addCategoryData.name"></el-input>
        </el-form-item>
        <el-form-item prop="profile" label="简介" label-position="top">
