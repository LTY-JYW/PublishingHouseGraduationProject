...
import { ref } from 'vue'
// 导入API
import { booksCategoryAPI } from '@/api/books'
import { userGetListNoPageAPI } from '@/api/user'
// 导入API类型
import type { BooksResListType, BooksCategoryType } from '@/api/books'
...
// 图书Id
const cid: number = Number(route.query.id)

// 获取图书列表提交变量
const page = ref<BooksCategoryType>({
    page: 1,
    itemsPerPage: 6,
    cid
})
...
// 获取图书列表函数
const getList = async () => {
    const { data: { data } } = await booksCategoryAPI(page.value)
...
<template>
    <div class="box">
        <div class="nav" @click="isSearch = false">
            ...
            <div class="nav-name">{{ booksListNow![0].cValue }}</div>
        </div>
        <div class="filters">
        ...
        <div class="main" @click="isSearch = false">
